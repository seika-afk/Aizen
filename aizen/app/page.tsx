...
export default function Home() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadedImage(data.url + "?t=" + Date.now());
    setOutputImage(null); 
  };

  // Called by Chat component whenever AI responds
  const handleAiResponse = (data: { imageUpdated?: boolean; outputUrl?: string }) => {
    if (data.imageUpdated && data.outputUrl) {
      setOutputImage(data.outputUrl + "?t=" + Date.now());
    }
  };

  const displayImage = outputImage ?? uploadedImage ?? null;

  if (isMobile === null) return null;
  if (isMobile) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-white">Currently not available on smaller devices.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full h-screen gap-2 p-2">
      ...
      <div className="flex-1 bg-[#383D3F] rounded-lg p-2 flex justify-center">
        ...
        <div className="absolute bg-[#383D3F] rounded-lg p-1 flex gap-4 text-white top-7 z-10">
          ...
          <span>
            <FontAwesomeIcon
              ...
              onClick={() => { setOutputImage(null); }} // reset to original
        ...
        <div className="bg-[#313638] w-full h-full rounded-lg flex items-center justify-center">
          <div className="bg-[#313638] pointer-events-none">
            <img
              id="src"
              className={`h-full object-cover pointer-events-none ${!displayImage ? "w-32" : "w-full"}`}
              src={displayImage || "file.svg"}
              alt="Main"
            />
          </div>
        </div>
      ...
      <div className="w-1/3 bg-[#383D3F] rounded-lg p-2 flex flex-col overflow-hidden">
        <Chat onAiResponse={handleAiResponse} />
      </div>
    </div>
  );
}
